...
class TangentCubeGeometry extends THREE.BufferGeometry {
	constructor(sideLength = 1, segmentsPerSide = 1) {
      ...
      let scale = Math.tan(Math.pow(Math.abs(unitUV), 1/8) * (Math.PI / 4));
      // let scale = Math.tan(Math.abs(unitUV) * (Math.PI / 4));
			scale += (1.0 / 9007199254740992.0) * scale; // correct tiny floating-point inaccuracies
			console.log(`n ${n}, unitUV ${unitUV}, cellSpaceUV ${cellSpaceUV}, scale ${scale}, cellSpaceUV * scale = ${cellSpaceUV * scale}`);
      return cellSpaceUV * scale;
		...
		function buildPlane(u: number, v: number, w: number, udir: number, vdir: number, wdir: number) {
			...
			for (let iy = 0; iy < segmentsPlusOne; iy++) {
				const y = tangentAdjustment(iy) * segmentSize;

				for (let ix = 0; ix < segmentsPlusOne; ix++) {
					// console.log(`    tangentAdjustment(${ix}) == ${tangentAdjustment(ix)} . Formula: (${tangentAdjustment(ix)} * ${segmentSize}) * ${udir} = ${tangentAdjustment(ix) * segmentSize * udir}`);
					const x = tangentAdjustment(ix) * segmentSize;
