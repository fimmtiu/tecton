...
import { TerrainChunk } from "./terrain_chunk";
import { VisualHelper } from "./visual_helper";

export { Terrain };

class Terrain {
  protected scene: THREE.Scene; // FIXME: Just for debugging. Can remove later.
  protected planet: Planet; // FIXME: Don't need this circular dependency long-term. Just for debugging.
  protected chunks: Array<TerrainChunk>;
  ...
  constructor(scene: THREE.Scene, planet: Planet) {
    ...
    this.planet = planet;
    this.chunks = [];
    this.visualHelper = new VisualHelper(scene, false, false);
  ...
  generateInitialChunks() {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      if (!(key in chunksByLocation)) {
        const chunk = new TerrainChunk(location);
